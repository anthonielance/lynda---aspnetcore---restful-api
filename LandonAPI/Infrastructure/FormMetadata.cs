...
namespace LandonAPI.Infrastructure
{
    public static class FormMetadata
    {
        public static Form FromModel(object model, Link self)
        {
            ...
            foreach (var prop in model.GetType().GetTypeInfo().DeclaredProperties)
            {
                ...
                var name = attributes.OfType<DisplayAttribute>()
                    .SingleOrDefault()?.Name ?? prop.Name.ToCamelCase();
        ...
        public static Form FromResource<T>(Link self)
        {
            ...
            if (!sortableProperties.Any() && !searchableProperties.Any()) return new Form { Self = self };
            ...
            foreach(var prop in sortableProperties)
            {
                ...
                orderByOptions.Add(new FormFieldOption { Label = $"Sort by {name}", Value = name });
                orderByOptions.Add(new FormFieldOption { Label = $"Sort by {name} descending", Value = $"{name} desc" });
            }
            ...
            if (searchableProperties.Any())
            {
                var applicableOperators = searchableProperties
                    .SelectMany(x => x.GetCustomAttribute<SearchableAttribute>().ExpressionProvider.GetOperators())
                    .Distinct();
